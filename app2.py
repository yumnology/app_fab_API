...
@app.route(BASE_URL + 'emotions/count', methods=['GET'])
def count_emotions():
    counts = db.session.query(
        EmotionEntry.emotion,
        func.count(EmotionEntry.emotion).label('count')
    ).group_by(EmotionEntry.emotion).all()

    count_dict = {emotion: count for emotion, count in counts}
